...
export const root = {
    register: async(
        { email, password, confirmation }: { email: string, password: string, confirmation: string}, 
        context: any
    ) =>  {
        ...
        return { ukey: user.ukey, tmpConfirmToken: "tmpConfirmToken" };
    },
    ...
    profile: async(
        ...
        const user = User.getByUserKey(ukey);
        if(user == undefined) {
            context.res.status(404);
